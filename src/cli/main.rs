...
fn main() {
    ...
    loop {
        ...
        match *args.get(0) {
            ...
            "stats" => { conn.stats() },
            "use" => { conn.use_tube(*args.get(1)) },
            "watch" => { conn.watch_tube(*args.get(1)) },
            "ignore" => { conn.ignore_tube(*args.get(1)) },
            "tube-stats" => { conn.tube_stats(*args.get(1)) },
            "quit" => {
                conn.quit();
                break;
            },
